...
{
    internal class RoleCreateRepository(Connection<RoleCreateDto> conn) : IRoleCreateRepository
    {
        public async Task<int> CreateAsync(RoleCreateDto Model, CancellationToken CancellationToken = default)
        {
            int RecordAffected = 1;

            Parameters parameters = new Parameters();
            parameters.NameProcedure = "USP_INS_ROLE";
